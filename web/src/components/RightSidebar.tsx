      ...
      <aside className={open
        ...
        <div className="bg-white rounded-2xl border border-[#e2e8f0] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#e2e8f0] flex items-center justify-between">
            <p className="text-xs font-bold text-[#94a3b8] uppercase tracking-widest">{t('rightSidebar.categoryManagement')}</p>
          ...
          <div className="divide-y divide-[#f1f5f9] max-h-80 overflow-y-auto">
            {loading ? (
              <p className="text-xs text-[#94a3b8] p-4 text-center animate-pulse">{t('common.loading')}</p>
            ) : categories.length === 0 ? (
              <p className="text-xs text-[#94a3b8] p-4">{t('rightSidebar.noCategories')}</p>
            ) : categories.map(cat => (
              <div key={cat.id} className="group flex items-center gap-2 px-3 py-2.5 hover:bg-[#f8fafc] transition-colors">
                ...
                <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEditCategory(cat)} title={t('common.edit')}
                  ...
                  <button onClick={() => onDeleteCategory(cat)} title={t('common.delete')}
        ...
        <div className="bg-white rounded-2xl border border-[#e2e8f0] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#e2e8f0]">
            <p className="text-xs font-bold text-[#94a3b8] uppercase tracking-widest">{t('rightSidebar.commonKeywords')}</p>
          </div>
          <div className="p-3 space-y-2">
            <div className="flex gap-1.5">
              <input
                ...
                placeholder={t('rightSidebar.addKeywordPlaceholder')}
            ...
            {keywords.length === 0 ? (
              <p className="text-xs text-[#94a3b8] text-center py-1">{t('rightSidebar.noKeywords')}</p>
            ) : (
              <div className="flex flex-wrap gap-1.5">
                {keywords.map(kw => (
                  <span key={kw} className="group flex items-center gap-1 pl-2.5 pr-1 py-1 bg-[#f0f9ff] border border-[#bae6fd] rounded-lg text-xs text-[#0ea5e9] font-medium">
                    {kw}
                    <button onClick={() => onDeleteKeyword(kw)} title={t('common.delete')}
